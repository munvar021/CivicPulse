...
const getCitizenDashboardData = asyncHandler(async (req, res) => {
  const citizen = await Citizen.findOne({ email: req.user.email }).select('_id').lean();
  ...
  const [total, inProgress, resolved, escalated, recentComplaints] = await Promise.all([
...
const getMyComplaints = asyncHandler(async (req, res) => {
  ...
  if (status && ['pending', 'assigned', 'in_progress', 'resolved', 'closed', 'rejected'].includes(status)) {
    query.status = status;
  }
